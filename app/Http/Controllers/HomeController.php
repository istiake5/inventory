...
class HomeController extends Controller
{
    ...
    { 
        ...
        if(Auth::user()->role_id > 2 && $general_setting->staff_access == 'own') {
            ...
            $revenue = Sale::whereDate('created_at', Carbon::today())->where('user_id', Auth::id())->sum('grand_total');
            ...
            $revenue = Sale::whereDate('created_at', Carbon::today())->where('user_id', Auth::id())->sum('grand_total');
            $revenue_week = Sale::whereBetween('created_at', [Carbon::now()->startOfWeek(), Carbon::now()->endOfWeek()])->where('user_id', Auth::id())->sum('grand_total');
            $revenue_month = Sale::whereMonth('created_at', date('m'))->whereYear('created_at', date('Y'))->where('user_id', Auth::id())->sum('grand_total');
            $lims_supplier_all = Supplier::where('is_active', true)->count();
            ...
            $week_purchase_return = ReturnPurchase::whereBetween('created_at', [Carbon::now()->startOfWeek(), Carbon::now()->endOfWeek()])->sum('grand_total');
            $revenue = $revenue - $return;
            $purchase = Purchase::whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->sum('grand_total');
            $week_purchase = Purchase::whereBetween('created_at', [Carbon::now()->startOfWeek(), Carbon::now()->endOfWeek()])->sum('grand_total');
            $profit = $revenue + $purchase_return - $purchase;
            $week_profit = $revenue_week + $week_purchase_return - $week_purchase;
            ...
            $pending_status = Sale::whereDate('created_at', Carbon::today())->where('sale_status','2')->count();
            $lims_sale_all = Sale::where('sale_status','3')->orderBy('id', 'desc')->get();
        ...
        //return $month;
        return view('index', compact('revenue', 'lims_sale_all', 'pending_status', 'week_profit', 'revenue_week', 'revenue_month', 'lims_supplier_all', 'purchase', 'expense', 'return', 'purchase_return', 'profit', 'payment_recieved', 'payment_sent', 'month', 'yearly_sale_amount', 'yearly_purchase_amount', 'recent_sale', 'recent_purchase', 'recent_quotation', 'recent_payment', 'best_selling_qty', 'yearly_best_selling_qty', 'yearly_best_selling_price'));
    }
